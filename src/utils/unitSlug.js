...
export function isUnitHashRef(ref = '') {
    return UNIT_HASH_PATTERN.test(String(ref || '').trim());
}

export function buildUnitRef(unitId) {
    return sha256Hex(`unit:${unitId}`).slice(0, 8);
}

export function resolveUnitListingRef(unit = {}) {
    const slug = String(unit.slug || unit.listingRef || '').trim();
    if (slug) {
        return slug;
    }

    if (unit.id != null) {
        return buildUnitRef(unit.id);
    }
